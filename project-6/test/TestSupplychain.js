...
contract('SupplyChain', function (accounts) {
	...
	it('Testing smart contract function sellItem() that allows a farmer to sell coffee', async () => {
		...
		assert.equal(item.productPrice, productPrice, 'Error: Invalid item price');
	...
	it('Testing smart contract function buyItem() that allows a distributor to buy coffee', async () => {
		...
		// Retrieve the just now saved item from blockchain by calling function fetchItem()
		const item1 = await supplyChain.fetchItemBufferOne.call(upc);
		const item2 = await supplyChain.fetchItemBufferTwo.call(upc);

		// Verify the result set
		assert.equal(item2.itemState, 4, 'Error: Invalid item State');
		assert.equal(item1.ownerID, distributorID, 'Error: Invalid owner.');
		assert.equal(
			item2.distributorID,
			distributorID,
	...
	it('Testing smart contract function receiveItem() that allows a retailer to mark coffee received', async () => {
		...
		// Verify the result set
		assert.equal(item1.ownerID, retailerID, 'Error: Invalid owner');
		assert.equal(item2.retailerID, retailerID, 'Error: Invalid retailer ID');
		assert.equal(item2.itemState, 6, 'Error: Invalid item State');
	...
	it('Testing smart contract function purchaseItem() that allows a consumer to purchase coffee', async () => {
		...
		// Verify the result set
		assert.equal(item1.ownerID, consumerID, 'Error: Invalid owner');
		assert.equal(item2.consumerID, consumerID, 'Error: Invalid consumer ID');
		assert.equal(item2.itemState, 7, 'Error: Invalid item State');
